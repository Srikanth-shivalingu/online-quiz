...
def login_view(request):
    ...
    return render(request,'online_quizz/login.html',{'error_message':error_message})
...
#     return render(request, 'online_quizz/register.html',  {'error_message': error_message})
...
def register_page(request):
    if request.method == "POST":
        ...
        if password_ == confirm_password_:
            if User.objects.filter(username=username_).exists():
                ...
                messages.error(request, 'Email already exists. Please enter a different email.')
                return redirect('online_quizz:register_page')
            else:
                user = User.objects.create_user(username=username_, email=email_)
                user.set_password(password_)
    ...
    return render(request, 'online_quizz/register.html')
@permission_required('online_quizz.view_course',login_url='online_quizz:login')
def course_list(request):
    technical_courses = Course.objects.filter(category='Technical')
    non_technical_courses = Course.objects.filter(category='Non-Technical')
    return render(request, 'online_quizz/course_list.html', {'technical_courses': technical_courses, 'non_technical_courses': non_technical_courses})


@permission_required('online_quizz.view_question',login_url='online_quizz:login')
...
def evaluate_results(request, course_id):
    if request.method == 'POST':
        ...
        result = Result.objects.create(user=request.user, group=Group.objects.get(name='students'), exam=course, marks=obtained_marks, percentage=percentage_value)
...
@permission_required('online_quizz.view_dashboard',login_url='online_quizz:login')
...
@permission_required('online_quizz.view_gradebook',login_url='online_quizz:login')
def gradebook(request):
    user_ = request.user
    results = Result.objects.filter(group__name='students', user=user_) # Filter results by group name 'students' and student
    ...
    return render(request, 'online_quizz/gradebook.html', {'profile_data': profile_data, 'overall_percentage': overall_percentage})

@permission_required('online_quizz.view_student_profile',login_url='online_quizz:login')
def teacher_view(request):
    students = User.objects.filter(groups__name='students')  # Get all users in 'students' group
    student_data = []

    for student in students:
        results = Result.objects.filter(group__name='students', exam__in=Course.objects.all(), user=student)
        total_marks = sum(result.marks for result in results)
        quizzes_attempted = results.count()
        overall_percentage = (total_marks / (quizzes_attempted * 100)) * 100 if quizzes_attempted > 0 else 0
...
@permission_required('online_quizz.view_teacher_course',login_url='online_quizz:login')
def teacher_courses(request):
    technical_courses = Course.objects.filter(category='Technical')
    non_technical_courses = Course.objects.filter(category='Non-Technical')
    return render(request, 'online_quizz/teacher_courses.html', {'technical_courses': technical_courses, 'non_technical_courses': non_technical_courses})

@permission_required('online_quizz.view_teacher_question',login_url='online_quizz:login')
def course_details(request, course_id):
    course = get_object_or_404(Course, id=course_id)
    questions = course.question_set.all()
    return render(request, 'online_quizz/course_details.html', {'course': course, 'questions': questions})

@permission_required('online_quizz.add_course',login_url='online_quizz:login')
def add_course(request):
    if request.method == 'POST':
        ...
        course = Course(course_name=course_name, description=description, total_marks=total_marks, category=category)
        course.save()
...
@permission_required('online_quizz.change_course',login_url='online_quizz:login')
...
@permission_required('online_quizz.delete_course',login_url='online_quizz:login')
...
@permission_required('online_quizz.add_question',login_url='online_quizz:login')
...
@permission_required('online_quizz.update_question',login_url='online_quizz:login')
...
@permission_required('online_quizz.delete_question',login_url='online_quizz:login')
